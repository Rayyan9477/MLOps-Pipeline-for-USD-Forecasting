...
class TwelveDataClient:
    ...
    def __init__(self, api_key: str = None):
        ...
        logger.info(
            f"Initialized TwelveDataClient for {self.symbol} with interval {self.interval}"
        )
    ...
    def fetch_time_series(
        ...
        logger.info(
            f"Fetching data: {self.symbol}, interval={self.interval}, outputsize={outputsize}"
        )
...
class DataQualityChecker:
    ...
    def check_null_values(df: pd.DataFrame, max_null_pct: float = None) -> bool:
        ...
        if not failed_columns.empty:
            logger.error(
                f"Null value check FAILED. Columns exceeding {max_null_pct}% threshold:"
            )
    ...
    def check_minimum_data_points(df: pd.DataFrame, min_points: int = None) -> bool:
        ...
        if len(df) < min_points:
            logger.error(
                f"Data points check FAILED. Got {len(df)}, need at least {min_points}"
            )
            return False
