...
class TestDataPipeline:
    ...
    def test_data_cleaning_handles_outliers(self, sample_forex_data):
        ...
        df_clean = cleaner.remove_outliers(
            df_with_outliers, columns=["close"], std_threshold=3
        )
...
class TestDriftMonitoring:
    ...
    def test_drift_detection_detects_shift(self, training_data):
        ...
        shifted_data = pd.DataFrame(
            {
                ...
                "close_rolling_mean_24": np.random.normal(
                    1.15, 0.005, 200
                ),  # Mean shifted
                "close_rolling_std_24": np.random.uniform(0.001, 0.003, 200),
        ...
        assert (
            "close_lag_1" in drifted_features
            or "close_rolling_mean_24" in drifted_features
        )
