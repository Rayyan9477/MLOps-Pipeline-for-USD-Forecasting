...
class ComponentTester:
    ...
    def test_model_artifacts(self):
        ...
        # Load and verify
        _model = joblib.load(model_path)  # noqa: F841
        _scaler = joblib.load(scaler_path)  # noqa: F841
    ...
    def test_data_extraction_module(self):
        ...
        assert TwelveDataClient is not None
        assert callable(DataQualityChecker.check_null_values)
    ...
    def test_data_transformation_module(self):
        ...
        assert engineer is not None, "FeatureEngineer not instantiated"
    ...
    def test_api_metrics_endpoint(self):
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/model/info", timeout=5
            )
            assert response.status_code == 200, \
                f"Model info endpoint failed: {response.status_code}"

            data = response.json()
            assert ('model_version' in data
                    or 'model_type' in data), \
                "Model info not in response"
        except Exception:
            response = requests.get(
                f"{self.api_base_url}/api/stats", timeout=5
            )
            assert response.status_code == 200, \
                f"API stats failed: {response.status_code}"
    ...
    def test_api_model_info_endpoint(self):
        ...
        assert response.status_code == 200, \
            f"Model info failed: {response.status_code}"

        data = response.json()
        assert ('model_version' in data
                or 'model_type' in data), "Model info not complete"

    def test_prometheus_metrics_endpoint(self):
        """Test Prometheus metrics endpoint."""
        response = requests.get(
            f"{self.api_base_url}/metrics", timeout=5
        )
        assert response.status_code == 200, \
            f"Prometheus metrics failed: {response.status_code}"
        assert ('predictions_total' in response.text
                or 'prediction' in response.text.lower()), \
            "Metrics not found"
    ...
    def run_all_tests(self):
        ...
        self.test("Data Transformation Module",
                  self.test_data_transformation_module)
        ...
        try:
            ...
            self.test("API Metrics Endpoint",
                      self.test_api_metrics_endpoint)
            self.test("API Model Info", self.test_api_model_info_endpoint)
            self.test("Prometheus Metrics",
                      self.test_prometheus_metrics_endpoint)
