...
def main():
    ...
    checker.check(
        len(env_files) == 0,
        f'No .env files in repository (found {len(env_files)})'
    )
    ...
    for pattern in secret_patterns:
        ...
        for py_file in py_files:
            # Skip venvs, tests, and health check itself
            skip_dirs = [
                'venv', 'site-packages',
                'test_', 'health_check.py'
            ]
            if any(x in str(py_file) for x in skip_dirs):
                continue
            try:
    ...
    checker.check(
        not found_secrets, 'No hardcoded secrets in code'
    )
    ...
    if metadata_file.exists():
        try:
            ...
            checker.check(
                test_mape < 20,
                f'Test MAPE < 20% ({test_mape:.2f}%)'
            )
            checker.check(
                test_rmse < 0.001,
                f'Test RMSE < 0.001 ({test_rmse:.6f})'
            )
            checker.check(
                features_count >= 20,
                f'Features count >= 20 ({features_count})'
            )
            checker.check(
            ...
            checker.check(
                'trained_at' in metadata
                or 'timestamp' in metadata,
                'Training timestamp present'
            )
        except Exception as e:
            checker.check(
                False, f'Model metadata valid: {str(e)}'
            )
    else:
        checker.check(
            False, 'Model metadata file exists', level='warning'
        )
    ...
    if workflow_file.exists():
        ...
        checker.check(
            'secrets.TWELVE_DATA_API_KEY' in content,
            'GitHub Secrets used'
        )
        checker.check(
            'extract' in content.lower(),
            'Extract step present'
        )
        checker.check(
            'transform' in content.lower(),
            'Transform step present'
        )
        checker.check(
            'train' in content.lower()
            or 'model' in content.lower(),
            'Training step present'
        )
    ...
    if dag_file.exists():
        ...
        checker.check(
            '0 */2 * * *' in content,
            'Airflow 2-hour schedule configured'
        )
        checker.check(
            '@dag' in content or 'DAG(' in content,
            'DAG decorator present'
        )
    ...
    for doc in required_docs:
        checker.check(
            (docs_dir / doc).exists(), f'{doc} present'
        )
    ...
    if checker.failed == 0:
        msg = "SYSTEM HEALTHY - READY FOR PRODUCTION"
        print(f"\n{GREEN}{BOLD}✅ {msg}{RESET}")
        return 0
    elif checker.failed <= 3:
        msg = "MINOR ISSUES DETECTED - REVIEW BEFORE PRODUCTION"
        print(f"\n{YELLOW}{BOLD}⚠ {msg}{RESET}")
        return 1
    else:
        msg = "CRITICAL ISSUES DETECTED - FIX BEFORE PRODUCTION"
        print(f"\n{RED}{BOLD}❌ {msg}{RESET}")
        return 1
